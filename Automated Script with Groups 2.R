...
dir.create("Forest Plots", showWarnings = FALSE)
dir.create("Funnel Plots", showWarnings = FALSE)
dir.create("Leave-One-Out", showWarnings = FALSE)
dir.create("Baujat Plots", showWarnings = FALSE)
dir.create("Influence Diagnostic", showWarnings = FALSE)
dir.create("Significance Tables", showWarnings = FALSE)
dir.create("Transitivity Graphs", showWarnings = FALSE)
dir.create("Network Tables", showWarnings = FALSE)
dir.create("Network Graphs", showWarnings = FALSE)
...
run_analysis <- function(file_path, results) {
  ...
  pdf(paste0("Influence Diagnostic/", outcome, ".pdf"), 15,20)
  plot(inf_obj)
  dev.off()
...
plot_forest_group <- function(outcomes, results, fig_no, title) {
  ...
  pdf(
    file = paste0("Figures/Figure_", fig_no, "_Forest.pdf"), width = 10,
    height = 3 * length(outcomes)  # otomatis tinggi
  )
  
  par(mfrow = c(length(outcomes), 1), mar = c(0,0,0,0))
...
files <- list.files("Datasets_Fix", pattern = "\\.xlsx$", full.names = TRUE)
